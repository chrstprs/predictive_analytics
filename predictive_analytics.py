...
Pada tahapan Gathering Data ini saya mengambil dataset dari Kaggle yang kemudian kmi upload ke Gdrive, untuk memudahkan akses dan pengelolaan. Dataset yang digunakan adalah dataset Pima Indians Diabetes Database, yang merupakan dataset populer untuk analisis prediksi penyakit diabetes.
...
Pada bagian Asesing Data ini saya menemukan bahwa dataset yang terdiri dari 768 baris dan 9 kolom (8 fitur dan 1 target Outcome) tampak lengkap tanpa nilai NaN, namun mengandung nilai 0 pada fitur seperti `Glucose`, `BloodPressure`, `SkinThickness`, `Insulin`, dan `BMI` yang secara medis tidak realistis, sehingga perlu dianggap sebagai missing values terselubung. Saya juga menemukan bahwa dataset ini tidak meimiliki redundansi, kemudian variasi nilai unik yang tinggi pada fitur kontinu seperti Glucose menawarkan informasi kaya untuk modeling.
...
Oleh karena itu, data ini adalah missing values dan akan diganti dengan NaN. Pada dataset ini saya mengganti nilai 0 dalam kolom Glucose, BloodPressure, SkinThickness, Insulin, dan BMI dengan NaN.
"""
...
"""**Insight**
 ...
 Pada bagian ini saya memvisualisasikan jumlah missing values (nilai yang hilang) dalam suatu dataset menggunakan plot batang (bar chart) dari library Plotly.
"""
...
Setelah data 0 yang tidak masuk akal diubah ke NaN, maka searang kita memiliki data yang missing value. Pada bagian ini saya melakukan pengubahan missing value menjadi data yang nilainya merupakan median untuk masing-masing outcome.
...
Hal ini saya lakukan agar datanya menjadi lebih cocok untuk masing-masing outcome.
"""
...
Pada bagian insulin ini saya mengubah data missing valuenya menjadi median untuk masing-masing outcome. Untuk data yang outcomenya adalah 0, nilai median adalah 102.5 , sedangkan untuk data insulin dengan outcome 1(diabetes), nilai median adalah 169.5
"""
...
def plot_distribution(data_select, size_bin):
    ...
    D = df[df['Outcome'] == 1]  # Diabetic dataset
    H = df[df['Outcome'] == 0]  # Healthy dataset

    # Mengambil Data yang Dipilih dan Menghapus Nilai NaN
    hist_data = [D[data_select].dropna().tolist(), H[data_select].dropna().tolist()]
...
# Call the function with a valid bin size
...
Pada bagian Glucose ini saya mengubah data missing valuenya menjadi median untuk masing-masing outcome. Untuk data yang outcomenya adalah 0, nilai median adalah 107 , sedangkan untuk data insulin dengan outcome 1(diabetes), nilai median adalah 140
"""
...
Pada bagian SkinThickness ini saya mengubah data missing valuenya menjadi median untuk masing-masing outcome. Untuk data yang outcomenya adalah 0, nilai median adalah 27.0 , sedangkan untuk data insulin dengan outcome 1(diabetes), nilai median adalah 32.0
"""
...
Pada bagian SkinThickness ini saya mengubah data missing valuenya menjadi median untuk masing-masing outcome. Untuk data yang outcomenya adalah 0, nilai median adalah 70.0 , sedangkan untuk data insulin dengan outcome 1(diabetes), nilai median adalah 74.5
"""
...
Pada bagian SkinThickness ini saya mengubah data missing valuenya menjadi median untuk masing-masing outcome. Untuk data yang outcomenya adalah 0, nilai median adalah 30.1 , sedangkan untuk data insulin dengan outcome 1(diabetes), nilai median adalah 34.3
"""
...
Pada bagian Feature Selection ini saya melakukan pemilihan fitur yang akan digunakan untuk pembuatan model berdasarkan korelasi antara fitur dengan outcome yang menjadi target. Kemudian saya memilih fitur yang korelasinya dengan outcome lebih dari 0.3, yaitu Glucose, Insulin, dan BMI.
"""
...
Pada tahapan normalisasi data, saya menggunakan bantuan dari library scikit learn untuk melakukan normalisasi dengan StandardScaler. Proses ini penting untuk memastikan bahwa fitur-fitur dengan rentang nilai yang berbeda (misalnya, Glucose yang berkisar 0-199 dan BMI yang berkisar 0-67) tidak mendominasi model machine learning hanya karena perbedaan skala.
"""
...
Pada langkah pembuatan model di bagian Train-Test Split Data ini saya melakukan pembagian dataset menjadi 70% untuk data latih, dan 30% untuk data uji. Total data uji adalah 231 data dengan 3 fitur
"""
...
Pada bagian cross-validation ini saya melakukan pemeriksaan awal performa dan keandalan model untuk memastikan model tidak hanya jago mengklasifikasikan satu jenis outcome.
"""
...
Untuk bagian ini saya menampilkan ROC Curve yang merupakan kurva yang bisa mengevaluasi kemampuan model dalam membedakan kelas.
